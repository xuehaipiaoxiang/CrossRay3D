...
# from RoiAwarePoolHelper import roiaware_pool3d_cuda
import RoiAwarePoolHelper
...
def points_in_boxes_cpu(points, boxes):
    ...
    RoiAwarePoolHelper.points_in_boxes_cpu(boxes.float().contiguous(), points.float().contiguous(), point_indices)
...
def points_in_boxes_gpu(points, boxes):
    ...
    RoiAwarePoolHelper.points_in_boxes_gpu(boxes.contiguous(), points.contiguous(), box_idxs_of_pts)

    return box_idxs_of_pts

def points_of_pillar_in_gt_gpu(points, boxes):
    """
    :param points: (B, M, 3)
    :param boxes: (B, T, 7), num_valid_boxes <= T
    :return box_idxs_of_pts: (B, M), default background = -1
    ...
    box_idxs_of_pts = points.new_zeros((batch_size, num_points), dtype=torch.int).fill_(-1)
    # roiaware_pool3d_cuda.points_in_boxes_gpu(boxes.contiguous(), points.contiguous(), box_idxs_of_pts)
    RoiAwarePoolHelper.points_of_pillar_in_gt_gpu(boxes.contiguous(), points.contiguous(), box_idxs_of_pts)
    return box_idxs_of_pts
...
class RoIAwarePool3dFunction(Function):
    ...
    def forward(ctx, rois, pts, pts_feature, out_size, max_pts_each_voxel, pool_method):
        ...
        RoiAwarePoolHelper.forward(rois, pts, pts_feature, argmax, pts_idx_of_voxels, pooled_features, pool_method)
    ...
    def backward(ctx, grad_out):
        ...
        RoiAwarePoolHelper.backward(pts_idx_of_voxels, argmax, grad_out.contiguous(), grad_in, pool_method)
