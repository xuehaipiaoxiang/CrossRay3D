...
from projects.mmdet3d_plugin.models.utils.misc import topk_gather, topk_gather_custom

from functools import reduce
from projects.mmdet3d_plugin.core.bbox.util import normalize_bbox
from projects.mmdet3d_plugin.models.utils.misc import pos2embed, SELayer_Linear, ORININ



@HEADS.register_module()
class SparseImgHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(SparseImgHead, self).__init__(init_cfg=init_cfg)
        ...
        # self.sparse_query_shape = [50, 50 ,3]
    ...
    def init_weights(self):
        super(SparseImgHead, self).init_weights()
    ...
    def generate_mape(self, pts_dict, img_dict, img_metas=None):
        ...
        if img_dict is not None:
            ...
            img_3d_pe = self.rv_embedding(img_3d_pe)
    ...
    def _loss_single_task(self,
        ...
        #cls_avg_factor = num_total_pos * 1.0 + num_total_neg * 1.0
...
########################### DN
    ...
    #     lidars2imgs = np.stack([meta['lidar2img'] for meta in img_metas])
    #     lidars2imgs = torch.from_numpy(lidars2imgs).float().to(ref_points.device)
    #     ref_points_ = ref_points * (ref_points.new_tensor(self.pc_range[3:]) - ref_points.new_tensor(self.pc_range[:3]))  \
    #           + ref_points.new_tensor(self.pc_range[:3])
        
    #     proj_points = torch.einsum('bnd, bvcd -> bvnc', \
    #                  torch.cat([ref_points_, ref_points_.new_ones(*ref_points.shape[:-1], 1)], dim=-1), lidars2imgs)
    ...
    #     sample_weight = rearrange(sample_weight, 'b (n t) c -> b n t c', n=6)
    #     gather_index = gather_index.to(torch.int64)
    #     query_sample_weight = topk_gather_custom(sample_weight, gather_index)
    #     query_sample_weight *= mask
    #     query_sample_weight = query_sample_weight.sum(dim=1)
    ...
    def forward_single(self, pts_dict, img_dict, img_metas):
        ...
        if not isinstance(img_metas, list):
            img_metas = [img_metas]
        _,  _, img_feats, img_3d_pe = self.generate_mape(pts_dict, img_dict, img_metas)
