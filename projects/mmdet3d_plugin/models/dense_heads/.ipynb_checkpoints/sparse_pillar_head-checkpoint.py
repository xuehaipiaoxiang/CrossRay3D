...
from projects.mmdet3d_plugin.models.utils.misc import pos2embed
...
class SparsePillarHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(SparsePillarHead, self).__init__(init_cfg=init_cfg)
        ...
        self.mape_bev_fusion_position = nn.Sequential(
            nn.Conv2d(self.hidden_dim*2, self.hidden_dim*2, 1, 1, padding='same', bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.hidden_dim*2, self.hidden_dim*2, 1, 1,  bias=True),
        )
    ...
    def init_weights(self):
        super(SparsePillarHead, self).init_weights()
        nn.init.uniform_(self.reference_points.weight.data, 0, 1)

    def generate_mape(self, pts_dict, img_dict, img_metas=None):
        ''' 
            Modal Alignment Positional Encoding
            to align sparse pts features && img features
        '''
        pts_feats, bev_2d_pe, img_feats, img_3d_pe = None, None, None, None
        ...
        if pts_dict is not None:
            ...
            coords_2d = self._bev_mape(downsample_spatial_shape)
            bev_2d_pe = coords_2d.unsqueeze(0)
            bev_2d_pe = self.mape_bev_fusion_position(bev_2d_pe)
            bev_2d_pe = rearrange(bev_2d_pe, 'b c h w -> b (h w) c')
            bev_2d_pe = self.bev_embedding(bev_2d_pe)
            bev_2d_pe = bev_2d_pe.repeat(batch_size, 1, 1)
            bev_2d_pe = topk_gather(bev_2d_pe, topk_indexes) if topk_indexes is not None else bev_2d_pe
            pts_feats = rearrange(pts_feats, 'b t c -> t b c')
            bev_2d_pe = rearrange(bev_2d_pe, 'b t c -> t b c')

        return (pts_feats, bev_2d_pe, img_feats, img_3d_pe)
    

    def _bev_mape(self, downsample_spatial_shape):
        '''
        ...
        coord_2d = torch.cat([batch_x[None], batch_y[None]], dim=0)

        coord_2d = coord_2d.view(2, -1).transpose(1, 0) # (H*W, 2)

        coord_2d = pos2embed(coord_2d, self.hidden_dim)
        coord_2d = rearrange(coord_2d, '(h w) c -> c h w', h =y_size)
        coord_2d = coord_2d.cuda()

        return coord_2d


    def prepare_query_weight(self, ref_points, img_metas, pts_dict, img_dict):
        ref_points = inverse_sigmoid(ref_points.clone()).sigmoid()
        sample_weight = pts_dict['sample_weight']
        batch_size = sample_weight.size(0)
        downsample_spatial_shape = (self.grid_size[0] // self.downsample_scale,
                                    self.grid_size[1] // self.downsample_scale)
        gather_index = (ref_points[..., 0:1] * downsample_spatial_shape[0]).int() + \
        (ref_points[..., 1:2] * downsample_spatial_shape[1]).int() * downsample_spatial_shape[0]
        gather_index = gather_index.to(torch.int64)
        gather_index = gather_index.unsqueeze(0).repeat(batch_size, 1, 1)
        query_sample_weight = topk_gather(sample_weight, gather_index)
        return query_sample_weight

    def prepare_split_scaler(self, targets, ref_points, query_sample_weight, groups):
        query_sample_weight = query_sample_weight.squeeze(-1)
        ref_points = inverse_sigmoid(ref_points.clone()).sigmoid()
        pc_range  = ref_points.new_tensor(self.pc_range)
        ref_points[..., :] = ref_points[..., :] * (pc_range[3:] - pc_range[:3])[None] + pc_range[None, :3]
        batch_size = query_sample_weight.size(0)
        ref_points = ref_points.unsqueeze(0).repeat(batch_size, 1, 1)
        (reject_scaler, ) = multi_apply(self._prepare_split_scaler_single, targets, ref_points, query_sample_weight)
        reject_scaler = torch.cat(reject_scaler).repeat(groups)
        reject_scaler = 1 / reject_scaler.sigmoid() # 1+e**(-x)
        return reject_scaler


    def _prepare_split_scaler_single(self, targets, ref_points, query_sample_weight):
        known_bbox_center = targets[:, :3].clone().cuda()
        known_bbox_scale = targets[:, 3:6].clone().cuda()
        diff = known_bbox_scale / 2
        nums_b = known_bbox_center.size(0)
        nums_q = ref_points.size(0)
        query_sample_weight = query_sample_weight.repeat(nums_b, 1)
        ref_points = ref_points.repeat(nums_b, 1, 1)
        known_bbox_center = known_bbox_center.unsqueeze(1).repeat(1, nums_q, 1)
        known_dist = torch.norm((ref_points - known_bbox_center), 2, -1)
        known_r = diff.min(dim=-1, keepdim=True)[0]
        weight_mask = known_dist <= known_r * self.gamma_r
        reject_scaler = (query_sample_weight * weight_mask).sum(dim=-1)
        reject_scaler /= known_r.squeeze(-1).pow(3)
        # reject_sclaer /= (4/3 * np.pi * known_r.squeeze(-1).pow(3))

        
        return (reject_scaler, )

    def prepare_for_dn(self, batch_size, reference_points, img_metas, pts_dict, img_dict):
        if self.training:
            ...
            if self.bbox_noise_scale > 0:
                # query_sample_weight = self.prepare_query_weight(reference_points, img_metas, pts_dict, img_dict)
                # known_scaler = self.prepare_split_scaler(targets, reference_points, query_sample_weight, groups)
                diff = known_bbox_scale / 2 + self.bbox_noise_trans
                rand_prob = torch.rand_like(known_bbox_center) * 2 - 1.0
                # rand_prob = rand_prob * known_scaler.unsqueeze(-1) * self.gamma_step
                ...
                # mask = torch.norm(rand_prob, 2, 1) > (self.split * known_scaler)
            ...
            mask_dict = {
                ...
                'know_idx': know_idx,
                'pad_size': pad_size,
                # 'known_scaler': known_scaler.mean()
            }
    ...
    def _dn_loss_single_task(self,
        ...
        # known_scaler = mask_dict['known_scaler']
        ...
        # cls_avg_factor = num_tgt * 3.14159 / 6 * self.split * self.split  * self.split * known_scaler.pow(3)
...
################### DN
    ...
    def forward_single(self, pts_dict, img_dict, img_metas):
        ...
        if not isinstance(img_metas, list):
            img_metas = [img_metas]
        pts_feats, bev_2d_pe, _, _ = self.generate_mape(pts_dict, img_dict, img_metas)

        reference_points = self.reference_points.weight
        # self._visualize(reference_points, choice='bev')
        reference_points, attn_mask, mask_dict = self.prepare_for_dn(len(img_metas), reference_points, img_metas, pts_dict, img_dict)
        ...
        outs_dec, _ = self.transformer(
                            pts_feats, query_embeds,
                            bev_2d_pe,
                            attn_masks=attn_mask, ## DN
                        )
...
################################## DN
    ...
    def _visualize(self, reference_points, choice='bev', lidar=None, imgs=None):
        '''
         to visualize reference_points LiDAR & Cameras
        
        '''
        assert choice in ('bev', 'image') ,'error of choices'
        if choice == 'bev':
            points = reference_points.detach().cpu()
            pc_range = torch.tensor(self.pc_range)
            points = points * (pc_range[3:6] - pc_range[0:3])[None] + pc_range[None, :3]
            points = points.numpy()
            i = np.random.randint(0,1000)
            fpath = "vis/pictures/reference_points/" + "topk900_" + str(i) +".png"
            index = np.array([0, 3], dtype=np.int32)
            xlim = pc_range.cpu().numpy()[index]
            ylim = pc_range.cpu().numpy()[index+1]
