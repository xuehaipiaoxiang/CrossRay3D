...
class FocalInstanceImgHead(AnchorFreeHead): 
    ...
    def __init__(self,
                 ...
                 embed_dims = 256,
                 down_stride = 16,
                 train_strategy = 0, # 0 := constant ratio && 1 := random && 2:=sinuous_strategy  CREATIVE
                 train_ratio=1.0,
                 infer_ratio=1.0,
                 topk_weight = 1.5,
    ...
    def loss(self,
        ...
        # bs, num_sample_tokens= topk_indexes.shape[:2]
        # num_tokens = int (num_sample_tokens // self.sample_ratio)
        # mask_x = topk_indexes.new_zeros((bs, num_tokens, 1), requires_grad = False,)
        # mask_src = topk_indexes.new_ones((bs, num_sample_tokens, 1), requires_grad = False)
        # mask_x.scatter_add_(1, topk_indexes, mask_src)
        # mask_x = rearrange(mask_x, 'b (n h w) c ->(b n) h w c',n = 6, w =w)
        # mask_x = [mask_x[i].cpu().data.numpy().astype(np.float64)
        ...
        for i, img_mt in enumerate(img_metas, 0):
            ...
            for j, file_name in  enumerate(filenames, 0):
                ...
                if sample_weight is not None:
                    ...
                    num_sample_tokens = int(sample_score.size(0) * 0.5)
