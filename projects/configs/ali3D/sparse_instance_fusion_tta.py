...
test_pipeline = [
    ...
    dict(
        ...
        img_scale=[(0, 120, 800, 440), (0, 130, 800, 450)],
        pts_scale_ratio=[0.99, 1.0, 1.01],
        flip=False,
        transforms=[
            dict(
            ...
            dict(type='CustomResizeCropFlipImageTTA', data_aug_conf = ida_aug_conf, training=False),
            dict(type='NormalizeMultiviewImage', **img_norm_cfg),
            dict(type='PadMultiViewImage', size_divisor=32),
            dict(
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            'img_backbone' : dict(lr_mult=0.25, decay_mult=2),
            'img_neck' : dict(lr_mult=0.25),
            'img_roi_head' : dict(lr_mult=0.25),
            'pts_backbone' : dict(lr_mult=0.25, decay_mult=2),
        }),
