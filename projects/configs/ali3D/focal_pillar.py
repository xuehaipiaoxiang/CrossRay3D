...
model = dict(
    ...
    pts_backbone=dict(
        ...
        point_cloud_range = point_cloud_range,
        voxel_size = voxel_size,
        # train_ratio = 0.6, # choose first 50
        # infer_ratio = 0.6, # diffusion strategy
    ),

    pts_bbox_head=dict(
        type='SparsePillarHead',
        in_channels=256,
        hidden_dim=256,
        grid_size = sparse_shape,
        bbox_coder=dict(
...
lr_config = dict(
    policy='cyclic',
    # target_ratio=(6, 0.0001), adapt to single GPU
...
resume_from = None
# resume_from = 'work_dirs/asf_pts_sparse_lidarOnly/epoch_8.pth'
workflow = [('train', 1)]
gpu_ids = range(0, 8)

# best setting1  24, 0.0001, 2, 
